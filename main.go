...
func buildGoogleUrls(searchTerm, countryCode, languageCode string, pages, count int) ([]string, error) {
	...
	if googleBase, found := googleDomains[countryCode]; found {
		for i := 0; i < pages; i++ {
			start := i * count
			scrapeURL := fmt.Sprintf("%s%s&num=%d&hl=%s&start-%d&f", googleBase, searchTerm, count, languageCode, start)
...
func scrapeClientRequest(searchURL string, proxyString interface{}) (*http.Response, error) {
	...
	if res.StatusCode != 200 {
		err := fmt.Errorf("scrapper received a non-200 status code suggestying a ban")
		return nil, err
	}
...
func main() {
	res, err := GoogleScrape("Ayush Ojha", "com", "en", nil, 1, 30, 10)
	if err == nil {
		for _, res := range res {
			fmt.Println(res)
		}
